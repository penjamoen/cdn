jQuery(function($) {
	...
	$('.edit-menu').live('click', function() {
		...
		gbox.show({
			type: 'ajax',
			url: 'route.php?mod=menumanager&act=editmenu&id=' + menu_id,
			buttons: {
				'Save': function() {
					$.ajax({
						type: 'POST',
	...
	$('.delete-menu').live('click', function() {
		...
		gbox.show({
			...
			buttons: {
				'Yes': function() {
					$.post('route.php?mod=menumanager&act=deletemenu', param, function(data) {
						if (data.success) {
							gbox.hide();
							li.remove();
						} else {
	...
	$('#form-add-menu').submit(function() {
		if ($('#menu-title').val() == '') {
			...
			$.ajax({
				type: 'POST',
				url: 'route.php?mod=menumanager&act=addmenu',
				data: $(this).serialize(),
				error: function() {
					gbox.show({
	...
	$('#add-group a').click(function() {
		gbox.show({
			type: 'ajax',
			url: 'route.php?mod=menumanager&act=addmenugroup',
			buttons: {
				'Save': function() {
	...
	$('#edit-group').click(function() {
		...
		inputgroup.focus().select().keydown(function(e) {
			if (e.which == 13) {
				...
				$.ajax({
					type: 'POST',
					url: 'route.php?mod=menumanager&act=editmenugroup',
	...
	$('#delete-group').click(function() {
		...
		gbox.show({
			...
			buttons: {
				'Yes': function() {
					$.post('route.php?mod=menumanager&act=deletemenugroup', param, function(data) {
						if (data.success) {
